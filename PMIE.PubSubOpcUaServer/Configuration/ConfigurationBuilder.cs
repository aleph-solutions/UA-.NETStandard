...
namespace PMIE.PubSubOpcUaServer.Configuration
{
    public class ConfigurationBuilder
    {
        ...
        public void Start()
        {
            ...
            var objFolderNodes = Browse(Opc.Ua.ObjectIds.ObjectsFolder);
            ...
            if (deviceSetExpNodeId != null)
            {
                ...
                foreach (var node in browseRes.Where(x => x.NodeClass == NodeClass.Object))
                {
                    ...
                    if (typeId.Equals(ExpandedNodeId.ToNodeId(PMI.TMCPlus.ObjectTypeIds.PMI_MachineModuleType, _browseSession.NamespaceUris)))
                    {
        ...
        private void SampleEvents()
        {
            ...
            _configurationClient.AddWriter(group, "EventWriter", dataset.Name, 1, out DataSetWriterDefinition writer);
            _configurationClient.AddWriter(group, "EventWriter2", dataset2.Name, 1, out DataSetWriterDefinition writer2);
            _configurationClient.AddWriter(group, "EventWriter3", dataset3.Name, 1, out DataSetWriterDefinition writer3);
        ...
        public void ConfigureMachineModule(NodeId machineModuleId, string machineName)
        {
            ...
            var jsonConfig = LoadJsonDataConfiguration("MachineModule");
            var typeId = GetTypeDefinition(machineModuleId);
            var datasetItems = InitializeItemList(machineModuleId, typeId);
            var objectIncluded = LoadItemList(datasetItems, jsonConfig, machineModuleId);
            if (objectIncluded)
            {
                ...
                //Prepare the writer
                _configurationClient.AddWriter(writerGroup, $"{machineName}", publishedDataSet.Name, $"{writerGroup.QueueName}", jsonConfig.KeyFrameCount, out DataSetWriterDefinition writer);
            }
            ...
            foreach (var subObjId in subObjects)
            {
                ...
                foreach (var conf in eventsConfiguration)
                {
                    ...
                    if (ObjectIncluded(conf, subObjId))
                    {
                        ...
                        _configurationClient.AddWriter(writerGroup, $"{machineName}.{conf.EventTypeName}", eventDataSet.Name, $"{writerGroup.QueueName}/{conf.EventTypeName}", 0, out writerEvent);
                    }
                }
            }
        ...
        private void InitializeEncodableTypes()
        {
            ...
            EncodeableFactory.GlobalFactory.AddEncodeableType(typeof(PMI.TMCPlus.PMI_BoMType));
            EncodeableFactory.GlobalFactory.AddEncodeableType(typeof(PMI.TMCPlus.PMI_BoMEntryType));
            ...
            EncodeableFactory.GlobalFactory.AddEncodeableType(typeof(PMI.TMCPlus.PMI_MaterialAttributesType));
            EncodeableFactory.GlobalFactory.AddEncodeableType(typeof(PMI.TMCPlus.PMI_MaterialLotAttributesType));
            EncodeableFactory.GlobalFactory.AddEncodeableType(typeof(PMI.TMCPlus.PMI_MaterialQualityType));
            EncodeableFactory.GlobalFactory.AddEncodeableType(typeof(PMI.TMCPlus.PMI_MaterialStateEnumeration));
        }
        ...
        private void ConfigureDefectSensors(NodeId folderId, string machineName)
        {
            ...
            foreach (var objItem in references.Where(x => x.NodeClass == NodeClass.Object))
            {
                ...
                if (typeId.Equals(ExpandedNodeId.ToNodeId(PMI.TMCPlus.ObjectTypeIds.PMI_DefectDetectionSensorType, _browseSession.NamespaceUris)))
                {
                    //Do things
                }
            }
        ...
        private void ConfigureMaterialStorageBuffers(NodeId folderId, string machineName)
        {
            ...
            var jsonConfig = LoadJsonDataConfiguration("MaterialBuffer");

            var references = Browse(folderId);
            foreach (var objItem in references.Where(x => x.NodeClass == NodeClass.Object))
            {
                var typeId = GetTypeDefinition(objItem.NodeId);
                if (typeId.Equals(ExpandedNodeId.ToNodeId(PMI.TMCPlus.ObjectTypeIds.PMI_MaterialStorageBufferType, _browseSession.NamespaceUris)))
                {
                    ...
                    var objectIncluded = LoadItemList(datasetItems, jsonConfig, objectId);
                    ...
                    if (objectIncluded)
                    {
                        ...
                        //Prepare the writer
                        _configurationClient.AddWriter(writerGroup, $"{machineName}.MaterialBuffers.{objItem.BrowseName.Name}", publishedDataSet.Name, $"{writerGroup.QueueName}/{objItem.BrowseName.Name}", jsonConfig.KeyFrameCount, out DataSetWriterDefinition writer);
                    }
        ...
        private void ConfigureMaterialLoadingPoints(NodeId folderId, string machineName)
        {
            ...
            var jsonConfig = LoadJsonDataConfiguration("MaterialLoadingPoint");

            var references = Browse(folderId);
            foreach (var objItem in references.Where(x => x.NodeClass == NodeClass.Object))
            {
                var typeId = GetTypeDefinition(objItem.NodeId);
                if (typeId.Equals(ExpandedNodeId.ToNodeId(PMI.TMCPlus.ObjectTypeIds.PMI_MaterialLoadingPointType, _browseSession.NamespaceUris)))
                {
                    var objectId = ExpandedNodeId.ToNodeId(objItem.NodeId, _browseSession.NamespaceUris);
                    ...
                    if (objectIncluded)
                    {
                        ...
                        //Prepare the writer
                        _configurationClient.AddWriter(writerGroup, $"{machineName}.MaterialLoadingPoints.{objItem.BrowseName.Name}", publishedDataSet.Name, $"{writerGroup.QueueName}/{objItem.BrowseName.Name}", jsonConfig.KeyFrameCount, out DataSetWriterDefinition writer);
                    }
        ...
        private void ConfigureMaterialOutputs(NodeId folderId, string machineName)
        {
            ...
            var jsonConfig = LoadJsonDataConfiguration("MaterialOutput");

            var references = Browse(folderId);
            foreach (var objItem in references.Where(x => x.NodeClass == NodeClass.Object))
            {
                var typeId = GetTypeDefinition(objItem.NodeId);
                if (typeId.Equals(ExpandedNodeId.ToNodeId(PMI.TMCPlus.ObjectTypeIds.PMI_MaterialOutputType, _browseSession.NamespaceUris)))
                {
                    var objectId = ExpandedNodeId.ToNodeId(objItem.NodeId, _browseSession.NamespaceUris);
                    ...
                    if (objectIncluded)
                    {
                        ...
                        //Prepare the writer
                        _configurationClient.AddWriter(writerGroup, $"{machineName}.MaterialOutputs.{objItem.BrowseName.Name}", publishedDataSet.Name, $"{writerGroup.QueueName}/{objItem.BrowseName.Name}", jsonConfig.KeyFrameCount, out DataSetWriterDefinition writer);
                    }
        ...
        private void ConfigureMaterialRejectionTraps(NodeId folderId, string machineName)
        {
            ...
            var jsonConfig = LoadJsonDataConfiguration("MaterialRejectionTrap");

            var references = Browse(folderId);
            foreach (var objItem in references.Where(x => x.NodeClass == NodeClass.Object))
            {
                var typeId = GetTypeDefinition(objItem.NodeId);
                if (typeId.Equals(ExpandedNodeId.ToNodeId(PMI.TMCPlus.ObjectTypeIds.PMI_MaterialRejectionTrapType, _browseSession.NamespaceUris)))
                {
                    var objectId = ExpandedNodeId.ToNodeId(objItem.NodeId, _browseSession.NamespaceUris);
                    ...
                    if (objectIncluded)
                    {
                        ...
                        //Prepare the writer
                        _configurationClient.AddWriter(writerGroup, $"{machineName}.MaterialRejectionTraps.{objItem.BrowseName.Name}", publishedDataSet.Name, $"{writerGroup.QueueName}/{objItem.BrowseName.Name}", jsonConfig.KeyFrameCount, out DataSetWriterDefinition writer);
                    }
        ...
        private void ConfigureProcessControlLoops(NodeId folderId, string machineName)
        {
            ...
            var jsonConfig = LoadJsonDataConfiguration("ProcessControlLoop");
            ...
            foreach (var objItem in references.Where(x => x.NodeClass == NodeClass.Object))
            {
                ...
                if (typeId.Equals(ExpandedNodeId.ToNodeId(PMI.TMCPlus.ObjectTypeIds.PMI_ProcessControlLoopType, _browseSession.NamespaceUris)))
                {
                    ...
                    if (objectIncluded)
                    {
                        ...
                        //Prepare the writer
                        _configurationClient.AddWriter(writerGroup, $"{machineName}.ProcessControlLoops.{objItem.BrowseName.Name}", publishedDataSet.Name, $"{writerGroup.QueueName}/{objItem.BrowseName.Name}", jsonConfig.KeyFrameCount, out DataSetWriterDefinition writer);
                    }
        ...
        private void ConfigureProcessItems(NodeId folderId, string machineName)
        {
            try
            {
                ...
                var jsonConfig = LoadJsonDataConfiguration("ProcessItem");

                var references = Browse(folderId);
                foreach (var objItem in references.Where(x => x.NodeClass == NodeClass.Object))
                {
                    var typeId = GetTypeDefinition(objItem.NodeId);
                    if (typeId.Equals(ExpandedNodeId.ToNodeId(PMI.TMCPlus.ObjectTypeIds.PMI_ProcessControlItemType, _browseSession.NamespaceUris)) ||
                        typeId.Equals(ExpandedNodeId.ToNodeId(PMI.TMCPlus.ObjectTypeIds.PMI_ProcessItemType, _browseSession.NamespaceUris)))
                    {
                        var objectId = ExpandedNodeId.ToNodeId(objItem.NodeId, _browseSession.NamespaceUris);
                        ...
                        if (objectIncluded)
                        {
                            ...
                            //Prepare the writer
                            _configurationClient.AddWriter(writerGroup, $"{machineName}.ProcessItems.{objItem.BrowseName.Name}", publishedDataSet.Name, $"{writerGroup.QueueName}/{objItem.BrowseName.Name}", jsonConfig.KeyFrameCount, out DataSetWriterDefinition writer);
                        }
        ...
        public void ConfigureObjectEvents(string machineName, string objectName, NodeId objectId, DataSetWriterGroup writerGroup)
        {
            ...
            foreach (var conf in eventsConfiguration)
            {
                if (ObjectIncluded(conf, objectId))
                {
                    ...
                    DataSetWriterDefinition writerEvent = new DataSetWriterDefinition();
                    _configurationClient.AddWriter(writerGroup, $"{writerGroup.GroupName}.{objectName}.{conf.EventTypeName}", eventDataSet.Name, $"{writerGroup.QueueName}/{objectName}/{conf.EventTypeName}", 0, out writerEvent);
                }
            }
